...
from lostarm.template.template_macro import remember_macro
...
class IfStack(VerbosePrint):
    ...
    def _same_filename(self, opcode: str, filename: str, lineno: int ) -> None:
        ...
        to be in the same file as the previous element.
        Note: this includes by extension @ELSE_IF@ type statements.
...
class PreProcessor(VerbosePrint,ABC):
    """
    ...
    def findFile(self, filename: str) -> str:
        ...
        """
        return self._search_path.find_relative_to( filename, self.cur_filename() )
    ...
    def _reset_filename_lineno(self) -> None:
        ...
        self.fatal_set_filename( tos.get_filename(), 1 )
    ...
    def is_eof(self):
        ...
        for entry in self._include_stack:
            if entry.lineno < len(entry.lines):
                return False
    ...
    def consume_macro(self, opening_line : str ) -> None:
        ...
        while True:
            ...
            if '@MACRO' in raw_line:
                self.verbose_print(0,"%s:%d: Macro(%s) Opened here" % (fn,ln,name) )
                self.syntax_error("@MACRO within a macro is not supported")
            lines.append(raw_line)
            if len(lines) >= self.reasonable_macro_length:
                self.verbose_print(0,"%s:%d: ERROR: Macro(%s) begins" % (fn,ln,name) )
                self.verbose_print(0,"%s:%d: ERROR: Macro(%s) is exceeded %d lines" % ( self.cur_filename(), self.cur_lineno(), name, self.reasonable_macro_length ))
                self.fatal("Sorry cannot continue")

        m = re_macro_end.match(raw_line)
        if m is None:
